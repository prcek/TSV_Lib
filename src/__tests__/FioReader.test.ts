...
test('My FioReader - GetLast', async () => {
  ...
  const frr_p = fr.getLast();
  expect(fetch).toHaveBeenCalledTimes(0);
  jest.runAllTimers();
  expect(fetch).toHaveBeenCalledTimes(1);
  const frr = await frr_p;
...
test('My FioReader - GetPeriods', async () => {
  ...
  const fr = new FioReader('test_token');
  const frr_p =  fr.getPeriods(new Date(), new Date());
  jest.runAllTimers();
  expect(fetch).toHaveBeenCalledTimes(1);
  const frr = await frr_p;
  expect(frr.accountStatement.info.accountId).toBe('23231');
...
test('My FioReader - getLastId', async () => {
  ...
  const frr_p = fr.getDayLastId(new Date("2019-01-31"));
  jest.runAllTimers();
  const frr = await frr_p;
  ...
  const frr2_p = fr.getDayLastId(new Date("2019-07-31"));
  jest.runAllTimers();
  const frr2 = await frr2_p;
...
test('My FioReader - GetPeriod Throttle', async () => {
  ...
  const dm = jest.spyOn(global.Date, 'now').mockImplementation(() =>
  new Date(0).valueOf()
  );
  
 
  const fr = new FioReader('test_token');
  const frr1_p = fr.getPeriods(new Date(), new Date());
  expect(setTimeout).toHaveBeenLastCalledWith(expect.any(Function),0);
  const frr2_p = fr.getPeriods(new Date(), new Date());
  expect(setTimeout).toHaveBeenLastCalledWith(expect.any(Function),30000);
  const frr3_p = fr.getPeriods(new Date(), new Date());
  ...
  jest.runAllTimers();
  const frr1 = await frr1_p;
  const frr2 = await frr2_p;
  const frr3 = await frr3_p;
  expect(fetch).toHaveBeenCalledTimes(3);
